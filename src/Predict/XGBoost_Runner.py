...
init()
xgb_ml = xgb.Booster()
xgb_ml.load_model('Models/XGBoost_68.6%_ML-4.json')
xgb_uo = xgb.Booster()
xgb_uo.load_model('Models/XGBoost_53.1%_UO-9.json')
...
def xgb_runner(data, todays_games_uo, frame_ml, games, home_team_odds, away_team_odds, kelly_criterion):
    ...
    for game in games:
        ...
        # Guard against None values in odds
        if home_team_odds[count] is not None and away_team_odds[count] is not None:
            ev_home = float(Expected_Value.expected_value(ml_predictions_array[count][0][1], int(home_team_odds[count])))
            ev_away = float(Expected_Value.expected_value(ml_predictions_array[count][0][0], int(away_team_odds[count])))
            
            # Calculate Kelly if odds exist
            ...
            # Fallback if odds are missing
            ev_home = 0
            ev_away = 0
            bankroll_fraction_home = bankroll_descriptor + "N/A (No Odds)"
            bankroll_fraction_away = bankroll_descriptor + "N/A (No Odds)"
